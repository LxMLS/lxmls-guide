...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/RNN2.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/RNN2_backprop.pdf}
...
problem of \textit{vanishing} and \textit{exploding} gradients are a well known
...
long range dependencies and are less sensitive to this phenomenon, most notably
the Long Short-Term Memory (LSTM) \citep{hochreiter1997long} and the Gated
Recurrent Unit (GRU) \citep{cho2014learning}, both of which use gating
mechanisms to control information flow across time steps. It is
...
The difficulty of training RNNs over long sequences was one of the key
motivations behind the Transformer architecture \citep{vaswani2017attention},
which replaces recurrence with self-attention and therefore has no path-length
dependence on sequence length during gradient flow.
...
\STATE Apply final non-linear transformation e.g. softmax 
   $$p(y_m=k|\mathbf{x}_{1:m+1}) = h_{km}^y  = \frac{\exp(z_{km}^y)}{\sum_{k'=1}^{K} \exp(z_{k'm}^y)}$$
\end{algorithmic}
\end{algorithm}
...
\label{algo:rnnbackprop}
   ...
   \STATE initialize parameters $\Theta$ randomly
	...
	\FOR{$m=1$ {\bfseries to} $M'$}
        ...
        keep not only $h_{km}^y$ but also, the RNN layer activations $h_{jm}$ and the embedding layer activations $z^e_{dm}$.
	\ENDFOR	
	\vspace{0.3cm}
    ...
    \STATE {Initialize the error at last layer for a CE cost and backpropagate it through the output linear layer:
        $$\mathbf{e}^m_{y} = (\mathbf{W}^y)^T \Big(\mathrm{\mathbf{1}}_{k(m)} - \mathbf{h}_{m}^y \Big).$$}
	\ENDFOR	
