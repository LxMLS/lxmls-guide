...
composing linear and non-linear transformations. This composition can be expressed in
the form of a \textit{computation graph} where each node performs one or more operations. The parameters of the model are the weights given to the different node inputs in the linear transformation nodes. This resembles vaguely human neural networks, hence the name artificial neural networks. 

Due to their compositional nature, gradient methods and the chain rule can be applied 
learn the parameters of these models. See Section \label{gradient_methods} for a
refresh on the basic concept. We will also refer to the gradient learning
methods introduced in Section \ref{s:me}. Today we will focus on
\textit{feed-forward networks}. we will extended today's class to
\textit{recurrent neural networks} (RNNs).  
...
toolboxes available that allow you to design custom computational graphs as e.g.
...
\begin{figure}[!h]
\centering
%\includegraphics[scale=0.2]{figs/deep_learning/log_linear.png}
\includegraphics[scale=0.7]{figs/deep_learning/LogLin.pdf}
%\put(-15,50){\color[rgb]{0,0,0}\makebox(0,0)[lb]{\smash{$r_2$}}}%
...
for the $m$-th training example $\mathcal{D}^m=\{\mathbf{x}, \mathbf{y}\}$ is also
shown.} 
\label{fig:LogLinear}
\end{figure}

%Recall the maximum entropy classifier in Section \ref{s:me}. This was a log-linear model Eq.\ref{eq:loglinear} that, given an input vector of features $\boldsymbol{f}(x,y)$, would predict the probability of that vector belonging to each of $K$ possible classes $c_k$. %$y_k$.  For the purpose of introducing deep learning, we will use a very similar model that is given by
%
A computation graph is just a way of expressing a multi-input multi-output function with a directed acyclic graph. Each node in the graph applies a function over the parent nodes outputs and passes the result to its child-nodes thus attaining very complex functions by \textit{composing} simpler functions. To introduce the concept we will work with a very simple example, a categorical distribution over $K$ classes parametrized as a log-linear model. This is given by
%
\begin{align}
p(y=c_k|{x}) & = \frac{1}{Z}\exp\left(\sum_{i=1}^{I} W_{ik} x_i + b_k\right),
...
$\mathbf{x} \in \mathbb{R}^{I}$ and assigns a probability over $K$ possible
classes. It is parametrized by weights and bias $\Theta=\{\mathbf{W},
...
\begin{equation}
p(y=c_k|{x}) \equiv \tilde{z}_k = \frac{\exp(z_k)}{\sum_{k'=1}^{K} \exp(z_{k'})}.
\label{eq:softmax}
\end{equation}

This is shown in Fig,~\ref{fig:LogLinear}. Note that in the following sections we will also use $\mathbf{z}$ and
$\tilde{\mathbf{z}}$ to denote the output of linear and non-linear functions respectively.
...
a min-batch of examples as opposed to the whole train set. More elaborated
learning rules as e.g. momentum or Adagrad are also often used. These require
as well the computation of the gradients.  

\subsection{Deriving Gradients in Computation Graphs using the Chain Rule}

\begin{figure}[!h]
\centering
%\includegraphics[scale=0.2]{figs/deep_learning/log_linear_BP.png}
\includegraphics[scale=0.7]{figs/deep_learning/loglin_color.pdf}
\caption{Forward-pass (blue) and Backpropagation (red) calculations to estimate the gradient of weight $W_{K2}$ of a log-linear model.}
...
graph seen in the last section (and represented in Fig. \ref{fig:LogLin}). To simplify notation, and without loss of generality, we will work with the 
classification cost of a in individual example 

\begin{align}
\mathcal{F}(\mathcal{D}^m;\Theta) 
= -\log p(y^m=c_{k(m)} | \mathbf{x}^m) 
\label{eq:CostLogPosExample}
\end{align}

where $\mathcal{D}^m=\{(x^m, y^m)\}$. Extending the gradients of
\ref{eq:CostLogPosExample} to $M$ examples as in \ref{eq:CostLogPos} simply
implies computing the average of the gradients.
...
derivative with respect to the weight $W_{ki}$. To do this we invoke the \textbf{chain rule} to split the derivative calculation into two terms at variable $z_{k'}$ of Eq.\ref{eq:linear} with $k'=1\cdots K$
%
\begin{align}
\frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial W_{ki}} & = \sum_{k'=1}^{K} { \color{red} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial z_{k'}^m} }{ \color{blue} \frac{\partial z_{k'}^m}{\partial W_{ki}}}
\label{eq:LogLingCR1}
\end{align}
%
We have thus transformed the problem of computing the derivative into computing two easier derivatives. Since $z_{k'}$ only depends on the weight $W_{ki}$ in a linear way (see the graph in Fig.\ref{fig:LogLinColor}), the second derivative in Eq.\ref{fig:LogLingCR1} is given by
\begin{align}
...
\end{equation}
%
Here $\mathrm{\mathbf{1}}_{k(m)} \in \mathbb{R}^{K}$ is a vector of zeros with a one in $k(m)$, which is 
the index of the correct class for the example $m$. In order to compute
the derivatives of the cost function with respect to the bias parameters $b_{k}$ we only need to compute one additional derivative
\begin{align}
...
An important consequence of the previous derivation is the fact that each 
...
functions. Let us think, for example of the following extension of the previous
model
...
$$p(y=c_k|{x}) \equiv \tilde{z}_k^N = \frac{\exp(z_k^N)}{\sum_{k'=1}^{K} \exp(z_{k'}^N)}$$
...
\includegraphics[scale=0.7]{figs/deep_learning/NN.pdf}
...
as recurrent neural networks (RNNs) with minor modifications.  
...
\begin{itemize}
\item The \textit{forward pass} step, where the input signal is injected though the network  in a forward fashion (see Alg.~\ref{algo:mlpforward})
...
%\includegraphics[scale=0.2]{figs/deep_learning/mlp_BP.png}
\begin{figure}
\centering
\includegraphics[scale=0.7]{figs/deep_learning/NN_backprop_colored.pdf}
\caption{MLP}
\label{fig:LayerP}
\end{figure}

In a generic MLP we would like to compute the values of all parameters $\Theta=\{\mathbf{W}^1, \mathbf{b}^1, \cdots \mathbf{W}^N, \mathbf{b}^N\}$. As explained previously we will thus need to compute the backpropagated error at each node $\partial \mathcal{F}(\mathcal{D}^m;\Theta)/\partial z_k^n$ and the coresponding derivative for the forward-pass $\partial z_k^n/\partial W_{ik}$ for $n = 1 \cdots N$. Fortunately, it is easy to spot a recursion that will allows us to compute these values for each node given all its child nodes. To spot it, we can start trying to compute the gradients one layer before the output layer (see Fig.~\ref{fig:LayerP}), i.e. layer $N-1$. We start by spliting at $N-1$
%
\begin{align}
...
\end{align}
%
We wont be so lucky in the next application of the chain rule. Looking at Fig.~\ref{fig:LayerP}, it is clear that the derivatievs at each node in layer $N$ will depend on all values of layer $N-1$. We thus have
%
\begin{align}
...
\end{align}
%
\noindent then we can relate this to the cost derivative with respect to the preceeding layer with
%
\begin{align}
...
\end{align}
%
where we have used the fact that we used sigmoid non-linearities to compute $\frac{\partial \tilde{z}_{k'}^{N-1}}{\partial z_{k'}^{N-1}}$. This formula can also be intepreted as tranversing the graph in Eq.~\ref{eq:BPderivation3} backwards,  multiplying at each node the back-propagated error by the derivative of its inputs and summing the errors propagated from different nodes. A mor formal expression of the algorithm can be found in Algorithm~\ref{algo:backprop}.
