...
graphs for GPUs. The two dominant frameworks today are
PyTorch\footnotemark\footnotetext{https://pytorch.org/} and
TensorFlow\footnotemark\footnotetext{https://www.tensorflow.org/}. JAX\footnotemark\footnotetext{https://github.com/google/jax} has also gained significant traction in the research community.
...
CUDA-compatible code for use with GPUs. A key property of Pytorch is that its
computation graphs are \textit{dynamic}, meaning the graph is defined at
runtime. This will be a key factor simplifying design once we
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/LogLin.pdf}
...
%
\begin{equation}
p(y=k|{x}) \equiv h_k = \frac{\exp(z_k)}{\sum_{k'=1}^{K} \exp(z_{k'})}.
\label{eq:softmax}
\end{equation}
%
Note that in the following sections we will also use $\mathbf{z}$ and
$\mathbf{h}$ to denote the output of linear and non-linear functions
...
elaborated learning rules such as momentum, Adagrad, or Adam \citep{kingma2014adam} are used. Adam is the most widely used optimizer in modern deep learning practice. Bear in mind
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/LogLin_color.pdf}
...
\frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial W_{ki}} & =
  \begin{cases}
      -(1 - h_k) x_i^m  &  \mbox{ if } k = k(m)\\
      -(-h_k) x_i^m     &  \mbox{ otherwise }.
  \end{cases}.
\label{eq:LogLingCR1}
\end{align}
%
...
\begin{equation}
\nabla_\mathbf{W}\mathcal{F}(\mathcal{D};\Theta) = -\frac{1}{M}\sum_{m=1}^{M} \Big(\mathrm{\mathbf{1}}^{y^m} - \mathbf{h}^m \Big) \left(\mathbf{x}^m\right)^T.
\label{gradWeigths}
\end{equation}
%
...
\begin{equation}
\nabla_\mathbf{b}\mathcal{F}(\mathcal{D};\Theta) = -\frac{1}{M}\sum_{m=1}^{M} \Big(\mathrm{\mathbf{1}}^{y^m} - \mathbf{h}^m \Big).
\label{eq:gradBias}
\end{equation}
...
\label{algo:mlpforward}
   ...
   \STATE {\bfseries input:} Input data vector $\mathbf{h}^{0}  \equiv \mathbf{x}$.

	\FOR{$n=1$ {\bfseries to} $N-1$}
     \STATE Apply  linear transformation
        $$z_j^n = \sum_{i=1}^{I} W_{ji}^n h_i^{n-1} + b_j^n$$
     \STATE Apply non-linear transformation e.g. sigmoid (hereby denoted $\sigma()$)
     $$h_j^n = \sigma(z_j^n)  = \frac{1}{1+\exp(-z_j^n)}$$

	\ENDFOR

\STATE Apply final linear transformation
   $$z_k^N = \sum_{j=1}^{J} W_{kj}^N h_j^{N-1} + b_k^N$$
\STATE Apply final non-linear transformation e.g. softmax
$$p(y=k|{x}) \equiv h_k^N = \frac{\exp(z_k^N)}{\sum_{k'=1}^{K} \exp(z_{k'}^N)}$$
...
\begin{figure}[!hb]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/NN2.pdf}
...
\begin{figure}[!h]
\centering
\includegraphics[scale=0.9]{figs/deep_learning/NN_backprop_colored2.pdf}
...
\end{align}
%
where, as in the case of the log-linear model, we have used the fact that the ouput of the linear layer $z_j^{N-1}$ only depends on $W_{ji}^{N-1}$. We now pick the left-most factor and apply the chain rule to split by the output of the non-linear layer $h_j^{N-1}$. Assuming that the non linear transformation is one-to-one, as e.g. a sigmoid, tanh, relu we have
%
\begin{align}
        {\color{red} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial z_{j}^{N-1}}} = {\color{red} \bigg(\sum_{j'=1}^{J} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial h_{j'}^{N-1}} \frac{\partial h_{j'}^{N-1}}{\partial z_{j}^{N-1}}\bigg)} = {\color{red} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial h_{j}^{N-1}} \frac{\partial h_{j}^{N-1}}{\partial z_{j}^{N-1}}}
...
%
\begin{align}
        {\color{red} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial z_{j}^{N-1}}} = {\color{red} \bigg(\sum_{k'=1}^{K} \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial z_{k'}^{N}} \frac{\partial z_{k'}^{N}}{\partial h_{j}^{N-1}}\bigg)\frac{\partial h_{j}^{N-1}}{\partial z_{j}^{N-1}}}
...
%
\begin{align}
        e^{N-1}_j = \bigg(\sum_{k'=1}^{K} e^N_{k'} \frac{\partial z_{k'}^{N}}{\partial h_{j}^{N-1}}\bigg)\frac{\partial h_{j}^{N-1}}{\partial z_{j}^{N-1}}.
...
%
\begin{align}
        \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial W_{ji}^{N-1}}  = {\color{red} e^{N-1}_j} {\color{blue} h_i^{N-2}},  \quad \frac{\partial \mathcal{F}(\mathcal{D}^m;\Theta)}{\partial b_{j}^{N-1}}  = {\color{red} e^{N-1}_j}
...
%
\begin{align}
        \mathbf{e}^{N} =  \mathrm{\mathbf{1}}^{y} - \mathbf{h}^N,
  \quad
        \frac{\partial z_{k'}^N}{\partial h_j^{N-1}} = W_{k'j}^N \quad \mbox{and} \quad
        \frac{\partial h_{j}^n}{\partial z_{j}^n} = h^n_{j}\cdot (1-h^n_{j}) \quad \mbox{with} \quad n \in \{1 \cdots N-2\}
...
\begin{align}
 h_j =
  &\begin{cases}
      z_j  &  \mbox{ if } z_j >= 0\\
      0  &  \mbox{ otherwise }.
  \end{cases} \quad \quad \frac{\partial h_{j}}{\partial z_{j}} = \begin{cases}
      1  &  \mbox{ if } z_j > 0\\
      0  &  \mbox{ otherwise }.
  \end{cases}
  \label{eqn:relu}
...
%
\begin{align}
        \mathbf{e}^{N-1} = \left(\mathbf{\mathrm{J}}_{h}^{N-1}\right)^T \left(\mathbf{\mathrm{J}}_{W}^N\right)^T \mathbf{e}^N.
\label{eq:BPderivation5}
\end{align}
%
...
, MLP of $N$ layers, with parameters $\Theta=\{\mathbf{W}^1, \mathbf{b}^1, \cdots \mathbf{W}^N, \mathbf{b}^N\}$, number of rounds $T$, learning rate $\eta$
   ...
   \STATE initialize parameters $\Theta$ randomly
	...
	\STATE Compute the {forward pass} for each of the $M'$ examples in batch $b$;
	 keep not only $p(y^m|\mathbf{x}^m) \equiv \mathbf{h}^{m,N}$ but also all the intermediate non-linear outputs $\mathbf{h}^{m,1} \cdots \mathbf{h}^{m,N}$.
	\ENDFOR	
	\vspace{0.3cm}
	...
	\FOR{$n=N$ {\bfseries to} $1$}
        ...
        $$\mathbf{e}^{m,N} = \Big(\mathrm{\mathbf{1}}_{k(m)} - \mathbf{h}^{m,N} \Big).$$}
		\ENDFOR	
        \ELSE
   		\FOR{$m=1$ {\bfseries to} $M'$}
        ...
        $$\mathbf{e}^{m,n} = \mathbf{e}^{m} \odot \mathbf{h}^{m,n} \odot (\mathbf{\mathrm{1}}-\mathbf{h}^{m,n}),$$
        where $\odot$ is the element-wise product and the $\mathbf{\mathrm{1}}$ is replicated to match the size of $\mathbf{h}^n$.
        ...
        $$\nabla_{\mathbf{W}^n}\mathcal{F}(\mathcal{D_b};\Theta)  = -\frac{1}{M'} \sum_{m=1}^{M'} \mathbf{e}^{m,n} \cdot \left(\mathbf{h}^{m,n-1}\right)^T,$$
...
PyTorch has become the dominant framework in NLP and deep learning research, and is our choice for these labs. TensorFlow (backed by Google) remains widely used in production settings. JAX is increasingly popular for research requiring custom gradient computations or hardware-level performance. Transitioning between frameworks is not very complicated, as the core primitives are relatively similar across them.
