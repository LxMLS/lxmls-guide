...
In the original paper for transformers \citep{vaswani2017attention}, either $\mathrm{FF}$ or $\mathrm{MHA}$ is named as a ``sub-layer``. Based on the naming, an dropout is applied to input $z$ that has been passed through a ``sub-layer``, before a residual connection is applied. Finally, a layer normalization is applied. Together, we can express the function as
