...
%
\begin{enumerate}
	\item You must create a class and make it inherit from the \texttt{MRJob} class, as we did in the line \texttt{class WordCount(MRJob):}.
...
Steps 1-3 are very similar to the word count code which we gave you. The main difference is that you are splitting the document into trimers instead of words. Remember to use the syntax \texttt{> results\_pt.txt} and \texttt{> results\_en.txt} when you run the two jobs, to output the results to appropriate files.

If your code is in file \texttt{trimercount.py}, inside the \texttt{big_data} folder, the two commands you need to type into the terminal to run things locally are:
...
\begin{verbatim}
python big_data/trimercount.py -r emr s3://lxmls-labs/en_perline01.txt --num-ec2-instances 10 --aws-region eu-west-1 > en.counts.txt

python big_data/trimercount.py -r emr s3://lxmls-labs/pt_perline10.txt --num-ec2-instances 10 --aws-region eu-west-1 > pt.counts.txt
\end{verbatim}
