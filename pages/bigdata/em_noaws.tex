\section{Extra Work: Distributed EM}
...
In the directory \texttt{distributed\_em} you will find a few data and code files:
...
distributed (this will be improved in the
next exercises).
...
For the moment, your \texttt{reduce} function should have a single emission of the form
...
In order to address this problem we need to introduce a new operation,
...
This cuts down the communication overhead drastically and also makes the \texttt{reduce}
...
in the \verb+map+ function. Emit only in the \verb+mapper_final+ method.

The \verb+reduce+ function should not need to be changed at all.
...
Our code can still be improved in two ways: (1) there is a single \verb+reduce+ call,
...
The previous code also had \verb+reduce+ use resources that grow with the number of
processes. This may still be too much if you use a thousand of machines. We
will also avoid this problem.

In order to parallelize \revb+reduce+, we need to start emitting partial results at a
...
Note how we included the name of the state in the key. For the transition counts and emission counts, we will need a nested \verb+for+ loop:
...
\begin{exercise}
Look in the file \verb+emission_snippets.py+. This contains the \verb+for+ loops above
and more.

Use these to improve your \verb+mapper_final+ function. Write the corresponding
\verb+reduce+ function. Do not change the keys used for the emission as they are
