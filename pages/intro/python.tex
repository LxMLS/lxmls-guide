...
Note that division always returns a float in Python 3: \texttt{3 / 5} gives \texttt{0.6}. To perform integer (floor) division, use \texttt{//}: \texttt{3 // 5} gives \texttt{0}.

Also, notice that the symbol \texttt{**} is used as the exponentiation operator, unlike other languages that use \texttt{\^}. In Python, \texttt{\^} means bitwise XOR, so double-check your code if it uses exponentiation and gives unexpected results.
...
\noindent In the following example, we use this module to inspect the \texttt{greet} function. Since Python 3.7, the built-in \texttt{breakpoint()} function is the preferred way to set a breakpoint — it calls \texttt{pdb.set\_trace()} by default but can be configured to use other debuggers:
