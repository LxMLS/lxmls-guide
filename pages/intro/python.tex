...
At this point you shoudl have installed the needed packages. You need also to
feel confortable with an IDE to edit code and an interactive command line. 
See previous sections for the details.
...
functionallity in your IDE. If you wish to run from a command line instead do
...
\item[In the interactive command line]--~~Start you interactive command line of
...
statement, e.g. \code{while}, \code{for} or other. All commands within a
...
An alternative to IDEs, that can also be useful for quick debugging in Jupyter, 
is the pdb module. This will stop the execution at a given point (called break-point) to get a quick glimpse of the variable structures and to inspect the execution flow of your program. The ipdb is an improved version of pdb that has to be installed
by separate. It provides additional functionalities like larger context wondows
variable auto complete and colors. Unfortunately ipdb has some compatibility
problems with Jupyter. We therefore recommend to use ipdb only in very spartan
configurations such as vim+ipdb as IDE replacement.
