...
The following install instructions are same as the one on GitHub, provided here for completeness.

\noindent The basic install instructions use \texttt{uv}. 
If you are on Linux or MacOS, install \texttt{uv} with \newline
\texttt{curl -LsSf https://astral.sh/uv/install.sh | sh}\newline
If you are on Windows, use search to find and start the \texttt{CMD} command line utility. Then call on it \newline
\texttt{powershell -ExecutionPolicy ByPass -c "irm https://astral.sh/uv/install.ps1 | iex"} to install \texttt{uv}.

If you don't have Python on your system or want to install a specific version (the labs require the Python version to be between 3.9 and 3.12) you can install it using \texttt{uv}:\newline
\texttt{uv python install 3.10} \newline

You can then install all required dependencies and create a virtual environment using the one-liner: \newline
\texttt{uv sync --extra cpu} \newline

If your device has access to an Nvidia GPU, you can replace \texttt{cpu} with \texttt{cu118}, \texttt{cu124}, or \texttt{cu126} depending on the CUDA version your GPU supports.
Use \texttt{nvidia-smi} or \texttt{nvcc --version} to check the CUDA version.

To activate your virtual environment with all installed dependencies on Linux or MacOS use \texttt{source .venv/bin/activate}.
To do so on Windows use \texttt{.venv\textbackslash Scripts\textbackslash activate}

Check if your virtual environment is active by running \texttt{python --version \&\& which python}. This should display the correct version of python with a path from within the current virtual environment.
