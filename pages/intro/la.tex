...
int64
\end{python}
(On your computer, particularly if you have an older computer, \code{int} might
...
\textbf{Important note:} with numpy, you must use \code{dot} to get matrix
...
\begin{equation*}
x_{n}=\sum\limits_{i=1}^{n-1}\alpha_{i}x_{i}
\end{equation*}
for some scalar values $\alpha_{1}, \ldots, \alpha_{n-1} \in \mathbb{R}$. 

\begin{itemize}
\item The {\bf rank} of a matrix is the number of linearly independent columns. 
...
\item Two vectors $x,y \in \mathbb{R}^{n}$ {\bf orthogonal} if $x^{T}y=0$. A square matrix $U \in \mathbb{R}^{n\times n}$
