...
\todo{o script usa 1000 mas pode ser ate 400 no test seq se for menos da erro no plot final.}
\begin{python}
...
state. Note the high probability of having Noun after Adjective, or of having Verb after Nouns or Pronouns, as expected.}
\end{figure}
...
\todo{NOTA-MA: a funcao viterbi\_decode\_corpus imprime muita infromaao. ZM: mts warnings apenas}
\begin{python}
...
\end{python}
\todo{NOTA-ZM: eu obtive 0.531 no viterbi}
...
predicted tags). You should get something like \ref{fig:cmuns}.
...
>>> plt.show()
\end{python}
\todo{the code gives an error when plotting the graph if we consider less than 400 samples for (test seq)}
