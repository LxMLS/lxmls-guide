...
observation sequence $x = x_1\ldots x_N$, we want to find the sequence of hidden states $y^* = y_1^* \ldots y_N^*$; that explains it ``best''.
...
P_{\mathrm{emiss}}(x_1 | c_k)
 \\
 \mathrm{viterbi}(i, c_k) &=& \displaystyle \left( \max_{c_l \in \statevocab}   P_{\mathrm{trans}}(c_k | c_l) \times \mathrm{viterbi}(i-1, c_l) \right) \times P_{\mathrm{emiss}}(x_i | c_k)  \label{viterbiRecursion}
 \\
 \mathrm{backtrack}(i, c_k) &=&  \displaystyle \argmax_{c_l \in \statevocab}  P_{\mathrm{trans}}(c_k | c_l) \times \mathrm{viterbi}(i-1, c_l) 
 \\
...
P_{\mathrm{emiss}}(x_1 | c_k)$
        ...
        \FOR{$i=2$ {\bfseries to} $N$}
         \FOR{$c_k \in \statevocab$ }
          ...
          \STATE $\mathrm{backtrack}(i, c_k) = \displaystyle \argmax_{c_l \in \statevocab} P_{\mathrm{trans}}(c_k | c_l) \times \mathrm{viterbi}(i-1, c_l)$
         \ENDFOR 
        \ENDFOR 
       \STATE 
  $\max_{y \in \statevocab^N} P(X=x,Y=y) := \max_{c_l \in \statevocab} P_{\mathrm{final}}(\text{\tt stop} | c_l) \times \mathrm{viterbi}(N, c_l)$        
...
the ones given.
\todo{NOTA-MA: O segundo dataset ainda não foi definido.}
\begin{python}
>>> hmm.train_supervised(simple.train, smoothing=0.1)
