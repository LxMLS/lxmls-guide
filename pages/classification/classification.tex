...
\todo{De acordo com a defenicao dada de produto kronecker, o vector $f(x,y)$ dado em
...
described in the next following).
\footnote{Actually, we are showing a more robust variant of the perceptron, 
which averages the weight vector as a post-processing step.}  \todo{\small O preceptron algorithm consiste no metodo de gradiente quando a função de custo ´e o erro quadratico, certo? Assim, pode ter varios passos (neste caso fixou-se o valor do passo em 1), 
...
\todo{Falta definir o sd ("`simple dataset"') anterirormente. Apenas se definiu o dataset da Amazon.}
...
\todo{NOTE-MA: A sigla SGD (stochastic gradient descent?) não foi definida; não e percebe o que é.}
\begin{enumerate}
