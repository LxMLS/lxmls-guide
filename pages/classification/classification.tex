...
Compare the
results achieved and separating hyperplanes found.
\end{exercise}
...
Other algorithms and software packages for training SVMs include SVMLight (\url{http://svmlight.joachims.org})
and LIBSVM (\url{http://www.csie.ntu.edu.tw/~cjlin/libsvm/}), which allow non-linear kernels. These are generally more suitable for smaller datasets.
Note that in modern NLP, SVMs have been largely superseded by neural classifiers (in particular fine-tuned Transformers such as BERT), which tend to outperform SVMs on most text classification benchmarks without requiring manual feature engineering.
...
In practice, all of the algorithms discussed in this chapter (Naive Bayes, Perceptron, MaxEnt/logistic regression, SVM) are available in \texttt{scikit-learn} (\url{https://scikit-learn.org}), which has become the standard Python library for classical machine learning. It provides efficient, well-tested implementations alongside tools for cross-validation, feature extraction, and pipeline construction.

Some standalone implementations and toolkits are also available:
\begin{itemize}
\item LIBSVM: \url{http://www.csie.ntu.edu.tw/~cjlin/libsvm/} (non-linear kernel SVMs)
\item MALLET: \url{http://mallet.cs.umass.edu/} (MaxEnt, sequence models, topic models in Java)
\end{itemize}
