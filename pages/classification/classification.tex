...
from $O(\exp(J))$ to $O(J)$, 
hence estimation of ${\hat P}(Y)$ and ${\hat P}(X|Y)$ becomes much simpler, as we shall see. 
...
gaussian have $\Sigma = 0.5$, together with the Bayes optimal decision boundary). 
...
mnbb = mnb.MultinomialNaiveBayes()
...
\section{Features and Linear Classifiers}\label{sec:linearclass}
...
property of $x$. 
For instance in the toy examples described above you can thing of X
...
\subsection{Margin Infused Relaxed Algorithm (MIRA)}
...
%MIRA resembles the stochastic subgradient algorithm for SVMs, 
%but has a mechanism for choosing the stepsize. 
...
Maximum entropy models are trained \emph{discriminatively}: this means that, instead of maximizing the \emph{joint} likelihood 
...
\subsection{Support Vector Machines}\label{sec:svms}
